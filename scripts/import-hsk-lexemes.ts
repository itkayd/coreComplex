...
/** Which HSK 3.0 bands to include, e.g. "1" or "1,2,3". */
const BANDS = (process.argv[4] ?? "1").split(",").map((b) => `new-${b.trim()}`);
...
const band = raw
  .filter((e) => (e.level ?? []).some((l) => BANDS.includes(l)))
...
const bandLabel = BANDS.map((b) => b.replace("new-", "")).join("+");
const header = `/**
 * HSK ${bandLabel} expansion set — derived, NOT authored here.
 *
...
console.log(`bands ${BANDS.join(", ")}: ${band.length} words`);
